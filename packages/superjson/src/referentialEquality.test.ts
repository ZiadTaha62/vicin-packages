...
import type { RecursiveCustomTransfomer } from './custom-transformer-registry.js';
...
function createEqualityTests() {
  ...
  function equalityTest(input: any) {
    const dedupeTrueResult = dedupeTrue.deserialize(dedupeTrue.serialize(input));
    const dedupeFalseResult = dedupeFalse.deserialize(dedupeFalse.serialize(input));
