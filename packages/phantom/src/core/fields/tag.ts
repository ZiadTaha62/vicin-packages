import type { __Phantom, __Tag } from '../symbols';
...
export namespace TagCore {
  ...
  export type OfIfExists<Tag extends string | symbol> = [Tag] extends [never] ? {} : Of<Tag>;

  /** Extract the tag from a type */
  export type TagOf<T> = T extends { [__Phantom]: { [__Tag]: infer Tag } } ? Tag : never;

  /** Check whether a type is tagged */
  export type HasTag<T, Ta extends string | symbol = string | symbol> = T extends {
    [__Phantom]: { [__Tag]: Ta };
  }
    ? true
    : false;
