...
import {
  type NonRecursiveCustomTransfomer,
  type RecursiveCustomTransfomer,
  type AnyCustomTransformer,
  CustomTransformerRegistry,
} from './custom-transformer-registry.js';
import { applyMeta, generateReferentialEqualityAnnotations, walker } from './plainer.js';
...
export default class SuperJSON {
  ...
  serialize(object: SuperJSONValue): SuperJSONResult {
    ...
    const ignored: any[] = [];
    const output = walker(object, identities, ignored, this, this.dedupe);
    ...
    if (ignored.length > 0) res.ignored = ignored;
  ...
  registerCustom(transformer: Omit<AnyCustomTransformer, 'name'>, name: string) {
