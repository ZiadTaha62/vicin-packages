import type { SuperJSONValue, JSONValue } from './types.js';
...
export class CustomTransformerRegistry {
  ...
  register<I, O extends JSONValue>(transformer: NonRecursiveCustomTransfomer<I, O>): void;
  register<I, O extends SuperJSONValue>(transformer: RecursiveCustomTransfomer<I, O>): void;
  register(transformer: AnyCustomTransformer) {
    this.transformers[transformer.name] = transformer;
  }

  findApplicable<T>(v: T) {
    return find(this.transformers, (transformer) => transformer.isApplicable(v)) as
