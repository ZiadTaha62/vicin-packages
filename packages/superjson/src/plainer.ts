...
import { transformValue, type TypeAnnotation, untransformValue } from './transformer.js';
...
export type MinimisedTree<T> = Tree<T> | { [k: string]: MinimisedTree<T> } | undefined;
...
function traverse<T>(
  ...
  walker: (type: any, path: string[], equalityGroup: ReferentialEqualityGroup | undefined) => void,
  ...
  const equalityGroup = equalityGroupsByPath.get(stringifyPathWithDepth(origin, samePathDepth));
  ...
  if (!isArray(tree)) {
    ...
    forEach(tree, (subtree, key) => {
      const parsedKey = parsePath(key, enableLegacyPaths(version), enableDepthSegment(version));
  ...
  if (children) {
    traverse(children, walker, equalityGroupsByPath, version, origin, samePathDepth + 1);
  }
...
export function applyMeta(json: any, meta: MetaObject, superJson: SuperJSON): any {
  //
  // Parsing and function declarations
  //
  const version = meta.v ?? 0;

  const { rootEqualities, equalityGroupsByPath, representativePaths } = parseReferentialEqualities(
    meta.referentialEqualities,
    version
  );

  const setReferentialEquality = (representativePath: string[], identicalPaths: string[][]) => {
  ...
  for (const path of representativePaths) {
    const equalityGroup = equalityGroupsByPath.get(path)!;
    setReferentialEquality(equalityGroup.representative, equalityGroup.duplicates);
  }
...
function parseReferentialEqualities(
  ...
  const rootEqualities =
    rootEqualityPaths?.map((path) => parsePath(path, legacyPaths, depthSegment)) ?? [];

  const equalityGroupsByPath = new Map<string, ReferentialEqualityGroup>();
  const representativePaths = new Set<string>();

  if (nonRootGroups) {
    for (const [representativePath, duplicateOriginalPaths] of Object.entries(nonRootGroups)) {
      const parsedRepresentative = parsePath(representativePath, legacyPaths, depthSegment);
      ...
      for (const duplicateOriginalPath of duplicateOriginalPaths) {
        group.duplicates.push(parsePath(duplicateOriginalPath, legacyPaths, depthSegment));
...
export function generateReferentialEqualityAnnotations(
  ...
  identitities.forEach((paths) => {
    ...
    if (representativePath![0].length === 0) {
      ...
      result[stringifyPathWithDepth(representativePath![0], representativePath![1])] =
        identicalPaths.map(([path, depth]) => stringifyPathWithDepth(path, depth));
    }
  });
...
const isPlainObjectOrArray = (object: any) => isPlainObject(object) || isArray(object);
...
export const walker = (
  ...
  if (isPlainObjectOrArray(object)) {
    ...
    forEach(object, (value, index) => {
      ...
      handleInnerAnnotations(innerAnnotations, index, recursiveResult.annotations);
    });
...
function handleInnerAnnotations(
  innerAnnotations: Record<string, MinimisedTree<TypeAnnotation>>,
  index: string,
  annotation: MinimisedTree<TypeAnnotation>
) {
  if (!annotation) return;
  const key = escapeKey(index);

  if (isArray(annotation)) {
    innerAnnotations[key] = annotation;
    return;
  }

  const innerKeys = Object.keys(annotation);
  if (innerKeys.length === 1) {
    const innerKey = innerKeys[0]!;
    innerAnnotations[key + '.' + innerKey] = annotation[innerKey];
  } else {
    innerAnnotations[key] = annotation;
  }
}
